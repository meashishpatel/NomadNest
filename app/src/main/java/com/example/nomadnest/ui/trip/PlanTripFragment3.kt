...
class PlanTripFragment3 : Fragment() {
    ...
    override fun onResume() {
        ...
        sharedViewModel.selectedBudget?.let { selected ->
            when (selected) {
                "Cheap" -> {
                    setupBudgetUI(binding.cheapbtn, selected)
                }
                "Economic" -> {
                    setupBudgetUI(binding.economicbtn, selected)
                }
                "Luxury" -> {
                    setupBudgetUI(binding.luxurybtn, selected)
                }
                "Flexible" -> {
                    setupBudgetUI(binding.flexible, selected)
                }
            }
        }
    }
    ...
    private fun setupBudgetUI(option: View, budgetValue: String) {
        ...
        binding.budgetTV.text = budgetValue
        enableNextBtn()
    }
